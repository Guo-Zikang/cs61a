...
def balanced(m):
    ...
    left_torque = length(left(m)) * get_size(end(left(m)))
    right_torque = length(right(m)) * get_size(end(right(m)))
...
def get_size(m):
    if(is_planet(m)): return size(m)
    return get_size(end(left(m))) + get_size(end(right(m)))
...
def totals_tree(m):
    ...
    return tree(get_size(m), [totals_tree(end(left(m))), totals_tree(end(right(m)))])
