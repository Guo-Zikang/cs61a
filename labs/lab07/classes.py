...
class Player:
    ...
    def play(self, card_index):
        ...
        self.hand = original_hand[:card_index] + original_hand[card_index + 1:]
        return original_hand[card_index]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for elem in player.deck.cards:
            elem.attack += other_card.attack
            elem.defense += other_card.defense
        copy_cards = opponent.deck.cards
        for i in range(orig_opponent_deck_length):
            if copy_cards[i].attack == other_card.attack or copy_cards[i].defense == other_card.defense:
                j = len(opponent.deck.cards) - orig_opponent_deck_length + i
                opponent.deck.cards = opponent.deck.cards[:j] + opponent.deck.cards[j + 1:]
